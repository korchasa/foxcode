...
- [x] NF-7 / NF-8 — `foxcode/.mcp.json` (CC plugin) uses `{ "mcpServers": { "foxcode": { "command": "npx", "args": ["-y", "foxcode-channel@0.18.0"] } } }` with no `cwd` and no `env`. Evidence: `foxcode/.mcp.json:1`; `scripts/cc-mcp-snippet.test.mjs` passes; `npm view foxcode-channel dist-tags.latest` returns `0.18.0`.

Phase 2 — Codex plugin marketplace:

- [x] NF-8 — Codex plugin payload generated by `scripts/build-plugin-payload.mjs` ships skills + Firefox extension + a Codex-shaped `.mcp.json` whose `foxcode = { command: "npx", args: ["-y", "foxcode-channel@0.18.0"] }`. Channel sources are NOT shipped — npx resolves them from npm at first launch. Evidence: `node --test scripts/codex-plugin-payload.test.mjs`, `scripts/codex-payload-pin.test.mjs`.
- [x] NF-8 — `codex plugin marketplace add` + `codex plugin add foxcode@korchasa` against the built payload succeeds in an isolated `CODEX_HOME`. Evidence: `node --test scripts/codex-plugin-install.test.mjs`.
- [x] NF-8 — installed Codex plugin's MCP server starts via npx and `codex mcp list --json` shows `foxcode` connected. Evidence: `node --test scripts/codex-plugin-mcp.test.mjs::installed codex mcp tools list`.
- [x] NF-8 — channel running under Codex plugin install reports the **user's project directory** (not plugin cache) for `status` (Q1 resolved positively — Codex preserves user-session cwd in the MCP child under npx). Evidence: `scripts/codex-plugin-mcp.test.mjs::status.projectDir == realpath(user_project_dir)`.

Phase 3 — OpenCode plugin:

- [x] NF-7 — OpenCode plugin (`@korchasa/foxcode-opencode`) emits an npx-shaped MCP snippet on `session.created` and CLI `setup --write-config`. `bundle/channel/` is no longer copied at prepack; bundle ships skills + extension only. Evidence: `opencode/lib/foxcode-mcp-entry.mjs`, `opencode/lib/foxcode-mcp-entry.test.mjs`, `opencode/test/pack.test.mjs` (no `bundle/channel/`), `opencode/test/plugin.test.mjs`, `opencode/test/cli.test.mjs` updated to assert the npx snippet shape.
- [x] NF-7 — OpenCode plugin starts the channel via `npx` and emits the snippet with `{env:PWD}` interpolation so `FOXCODE_PROJECT_DIR` matches the user's project dir (Q3 resolved). Evidence: `opencode/test/cli.test.mjs::setup --write-config patches project opencode.json` asserts `command[0]=="npx"` and `command[2]` matches `foxcode-channel@<ver>`.

Cross-phase / hygiene:

- [x] Phase 0 baseline = `<pinned>` = `0.18.0` (P0.9 promoted to `latest`). Evidence: `npm view foxcode-channel dist-tags.latest` returns `0.18.0`.
- [x] Release lockstep verified. `scripts/release.sh X.Y.Z` bumps all JSON `.version` fields AND rewrites every `foxcode-channel@…` literal (CC `foxcode/.mcp.json`, OpenCode emitter `opencode/lib/foxcode-mcp-entry.mjs`, Codex payload builder `scripts/build-plugin-payload.mjs`). Evidence: `node --test scripts/release-sh.test.mjs` (`always bumps the JSON + pin-literal lockstep file-set`).
- [x] Smoke test stays green. Evidence: `FOXCODE_SMOKE=1 bash scripts/check.sh` runs `scripts/test-npx-channel.sh` which validates `npx -y foxcode-channel@0.18.0 --version` in isolated HOME.
- [x] CI publish pipeline restored. `.github/workflows/ci.yml::auto-release` publishes the channel to npm on every push to main with conventional-commit prefixes; `channel-publish` / `channel-deprecate` jobs provide manual rc + deprecate paths. Evidence: `node --test scripts/ci-yml-publish.test.mjs`.
- [x] Docs synced.
  - [x] `README.md`: install snippets for CC / Codex / OpenCode all show npx form; migration note for users with hand-rolled `~/.codex/config.toml`.
  - [x] `documents/requirements.md` NF-7/NF-8 acceptance updated: marketplace install path checked via npx-based payload; channel no longer bundled in OpenCode tarball.
  - [x] `documents/design.md` § Distribution & Setup: new top-level subsection «MCP server: npm-distributed channel via npx (cross-IDE)»; CC / Codex / OpenCode sub-sections rewritten around it.
  - [x] `AGENTS.md` Key Decisions: Codex env-var bullet rewritten to mark the glob workaround as removed; Codex `[mcp_servers]` TOML bullet rewritten to reflect that `npx` is `execvp`'d directly (no shell expansion); npx-distribution decision documented under «MCP server distributed via npm, launched via `npx` across all IDEs».
