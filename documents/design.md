...
- **`server.mjs`** - MCP server: WebSocket bridge, tool dispatch (`status`, `launchBrowser`, `evalInBrowser`), graceful shutdown (stdin close / SIGTERM / SIGINT -> terminate managed Firefox process group, terminate WS clients, close server, exit). Reads name/version from own `package.json` at runtime (single source of truth). HTTP endpoints: `GET /` serves info page (live status via polling), `GET /status` returns `{connectedClients}` JSON. CLI flag `--launch-foreground` enters supervised launch mode for `scripts/dev.sh` (no MCP stdio).
- **`lib.mjs`** - Shared logic: ID generation, message builders, tool definitions (3 tools), port management (`createHttpServer`, `portStorage`), password management (`passwordStorage`), `buildConnectionPage` (info page HTML with polling JS). File I/O limited to `portStorage` (`~/.foxcode/port`) and `passwordStorage` (`~/.foxcode/password`)
...
- **Tools exposed:**
  - `status()` - server telemetry (port, projectDir, uptime, connectedClients, pendingRequests, nodeVersion, serverVersion, pid, pluginRoot, launchMode, client). Always works, no browser required.
  - `launchBrowser({timeout?, headless?})` - launch Firefox with the FoxCode extension via `web-ext run`. Blocks until the extension connects (or the timeout expires). Returns `{status: "connected"|"already-connected"|"already-running"|"timeout"|"error", pid?, port?, reason?}`. Firefox lifecycle is tied to the channel: SIGTERM/SIGINT/stdin-EOF on the channel terminates the launched web-ext process group.
...
- **Deps:** `@modelcontextprotocol/sdk`, `ws`. `web-ext` is fetched on demand via `npx -y web-ext run …` — no direct dependency in `package.json`.
...
- **Structure:** `.claude-plugin/marketplace.json` (repo root) -> `foxcode/` (plugin dir, skills-only payload + `.mcp.json` pointer to the npm channel). The Firefox extension ships inside `foxcode-channel` itself — IDE plugin payloads no longer carry `extension/`.
- **Plugin contents:** `.claude-plugin/plugin.json` (manifest), `.mcp.json` (MCP server config), `skills/foxcode-run-project-profile/SKILL.md`, `skills/foxcode-run-user-profile/SKILL.md`. Channel sources live in `foxcode/channel/` in the repo (source of truth for the npm publish, including the bundled extension via `foxcode/channel/prepack.mjs`) and are fetched from npm at MCP launch time.
...
- **Launch skills:** `/foxcode:foxcode-run-project-profile` collapses to two MCP calls (`status` then `launchBrowser`). `/foxcode:foxcode-run-user-profile` keeps its manual `about:debugging` flow (cannot be moved into MCP). Skills no longer ship Python helpers — `foxcode/skills/foxcode-run-project-profile/scripts/` has been removed; Firefox launch lives entirely in the channel.
...
- **Package layout:** `index.mjs` (plugin entry), `lib/` (paths, seed-skills, mcp-snippet, patcher, exec, foxcode-mcp-entry, prereq, skill-frontmatter), `bin/foxcode-opencode.mjs` (CLI), `prepack.mjs` (bundle assembly), `bundle/` (created at pack time, contains `skills/` only — neither the channel nor the Firefox extension is bundled; they are fetched from npm at first MCP launch via the bundled `foxcode-channel` package).
- **Plugin route:** user adds `"plugin": ["@korchasa/foxcode-opencode"]` to `opencode.json`; OpenCode auto-installs via Bun. On `session.created` (earliest plugin-callable hook) the plugin (a) symlinks bundled SKILL.md dirs into `~/.config/opencode/skills/foxcode-run-{project,user}-profile/`, (b) emits an MCP snippet to stderr exactly when `mcp.foxcode` is missing from project + global `opencode.json`. The snippet is built by `lib/foxcode-mcp-entry.mjs::buildFoxcodeMcpEntry()` and has shape `{type:"local", command:["npx","-y","foxcode-channel@<pinned>"], environment:{FOXCODE_PROJECT_DIR:"{env:PWD}"}, enabled:true}`. Snippet emitted at most once per process. No `~/.foxcode/opencode-plugin-dir` handoff file is written — extension resolution lives entirely in the channel.
- **CLI route (one-shot):** `npx -y @korchasa/foxcode-opencode setup [--write-config]`. Same actions as the plugin, plus `--write-config` patches `opencode.json` directly (plain JSON only; refuses files with `//` or `/*` comments). `uninstall` removes seeded symlinks but never auto-removes `mcp.foxcode` (avoids destructive config mutation). `doctor` prints diagnostics.
...
- **Bundle exclusions:** `prepack.mjs` excludes `node_modules/`, `.foxcode/`, `build/`, `.DS_Store` from the copied skill trees. Channel and extension are never bundled.
...
- **Strict-MCP-host acceptance (Tier 1+2):** `opencode/test/acceptance/strict-mcp-host.test.mjs` spawns `server.mjs` against a custom NDJSON parser that treats any non-JSON line on stdout as a fatal protocol violation (modelling `codex exec --experimental-json`, not the lenient Claude Code / OpenCode parsers used by `mcp.test.mjs` and `bridge.test.mjs`). Exercises `launchBrowser` via a PATH-stub `npx` that prints `web-ext`-style banners and exits non-zero, plus a PATH-stub `firefox` so `findFirefox()` succeeds without a real binary. Asserts (a) zero non-JSON frames reach the host, (b) the channel survives `launchBrowser` and answers a follow-up `status`, (c) child stdout is forwarded to channel stderr so diagnostics remain visible. Catches the regression class behind `documents/tasks/2026/06/launchbrowser-closes-mcp-transport-under-codex-exec.md` without requiring real Firefox or LLM tokens.

### Tertiary: Codex (NF-8)
- **Repo-scoped install:** `.codex/config.toml` declares project-scoped `mcp_servers.foxcode` with the npx form (`command = "npx"`, `args = ["-y", "foxcode-channel@<pinned>"]`); `.agents/skills/foxcode-run-{project,user}-profile/` exposes launch skills; `.agents/skills/foxcode-{acceptance,distribution}-testing/` exposes project QA; `.agents/skills/foxcode-usage-analysis/` analyzes historical Codex/Claude usage; `.agents/skills/foxcode-skill-qa/` quality-tests the user-facing launch skills via fresh subagents (static review + cold trigger probes + optional real-Firefox behaviour probes). `.claude/skills` is a symlink to `../.agents/skills` so Claude Code and Codex share one skill source.
- **Marketplace install:** `codex plugin marketplace add korchasa/foxcode` registers the marketplace and caches the static payload (skills only — no `extension/`, no `channel/`) under `~/.codex/plugins/cache/korchasa/foxcode/<version>/`. The Firefox extension ships inside `foxcode-channel` and is reached on the first `npx -y foxcode-channel@<pinned>` invocation. The MCP server itself is declared in `~/.codex/config.toml` with the npx pin (Codex does not start MCP servers declared in plugin `.mcp.json` — upstream issue #19372). The payload is built by `scripts/build-plugin-payload.mjs` (`RUNTIME_DIRS = ['skills']`); `CHANNEL_SPEC` (= `foxcode-channel@<pinned>`) is bumped in lockstep by the release flow.
