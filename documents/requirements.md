...
- **Acceptance:**
  ...
  - [x] Launch scripts bundle Python utilities: `resolve_env.py` (Firefox+extension discovery + `.foxcode/config.json` cache; no port/password), `launch_firefox.py` (detached web-ext launch + PID lifecycle, `--port`/`--password` passed by skill from live `status`; `--foreground` for dev supervision). Evidence: `foxcode/skills/foxcode-run-project-profile/scripts/resolve_env.py`, `foxcode/skills/foxcode-run-project-profile/scripts/launch_firefox.py`
  ...
  - [x] Project-profile launch purges any staged macOS Firefox update markers (`update.status`, `update.version`, `update.mar`, `Updated.app`, `active-update.xml`) under `~/Library/Caches/Mozilla/updates/.../0/` and SIGTERMs any `org.mozilla.updater` process holding the FoxCode start URL, then proceeds to launch unconditionally. Evidence: `foxcode/skills/foxcode-run-project-profile/scripts/launch_firefox.py::_prepare_firefox_for_launch`, `foxcode/skills/foxcode-run-project-profile/scripts/test_launch_firefox.py::TestFirefoxUpdatePreparation`
...
- **Acceptance:**
  ...
  - [x] `prepack.mjs` syncs version from `foxcode/.claude-plugin/plugin.json`, copies `foxcode/extension/` and `foxcode/skills/foxcode-run-{project,user}-profile/` into `bundle/`. Channel sources are NOT bundled — they are pulled from npm by `npx` at first launch. Evidence: `opencode/test/pack.test.mjs`
