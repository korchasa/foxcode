...
- **Acceptance:**
  ...
  - [x] Launch scripts bundle Python utilities: `resolve_env.py` (Firefox+extension discovery + `.foxcode/config.json` cache; no port/password), `launch_firefox.py` (web-ext launch + PID lifecycle, `--port`/`--password` passed by skill from live `status`). Evidence: `foxcode/skills/foxcode-run-project-profile/scripts/resolve_env.py`, `foxcode/skills/foxcode-run-project-profile/scripts/launch_firefox.py`
