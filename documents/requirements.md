...
- **Same-folder shared browser (Acceptance):**
  - [x] N MCP servers in the SAME project dir share ONE Firefox; a 2nd `launchBrowser` never kills the 1st browser. The destructive port-mismatch kill is removed; a live browser yields a `reuse` verdict regardless of port. Evidence: `foxcode/channel/launch/spawn.mjs:163` (`handleExistingProcess` returns `reuse`/`spawn`, no mismatch kill), `foxcode/channel/launch/tool.test.mjs` (`reuse verdict + own client connects → already-running, no spawn`; `verdict reuse for a live browser on a DIFFERENT port — NEVER kills it`)
  - [x] Discovery via folder-scoped registry `<projectDir>/.foxcode/sessions.json` (ports + pids only, no secret) relayed through `pong.siblings`; extension connects to siblings reusing the machine-global password. Evidence: `foxcode/channel/launch/registry.mjs`, `foxcode/channel/launch/registry.test.mjs` (`entries contain ports + pids only — no password/secret`), `foxcode/channel/server.mjs:80,214,344` (register/advertise/unregister), `foxcode/channel/lib.mjs::buildPongMessage` (`siblings`), `foxcode/extension/background/background.js:237` (connect to siblings), `foxcode/extension/background/background.js:376` (5 s periodic re-ping)
  - [x] Folder isolation: a folder's browser only learns same-folder ports (registry is folder-scoped; no port-range scan). Evidence: `foxcode/channel/launch/registry.mjs::registryPath`, `foxcode/channel/launch/registry.test.mjs` (`registryPath points at <projectDir>/.foxcode/sessions.json`); manual 2-folder check (DoD)
  - [x] Concurrent same-folder launches do not double-spawn — crash-safe launch lock `<projectDir>/.foxcode/launch.lock` (holder pid + mtime, 60 s TTL); the loser reuses the winner's browser. Evidence: `foxcode/channel/launch/tool.mjs:83` (`acquireLaunchLock`), `foxcode/channel/launch/tool.test.mjs` (`live launch.lock holder → loser does not spawn`; `stale launch.lock (dead holder) → proceeds to spawn`)
  - [x] Owner HARD crash → next `launchBrowser` reaps the orphan (`browserPid` alive, `ownerPid` dead) and relaunches; no leaked Firefox, no profile-lock deadlock. PID file is 3-line (`browserPid`/`port`/`ownerPid`), legacy 2-line treated as healthy. Evidence: `foxcode/channel/launch/spawn.mjs:177` (orphan reap), `foxcode/channel/launch/spawn.test.mjs` (`reaps an orphan (browser alive, owner dead) → kills group, verdict spawn`; `legacy 2-line pid file … reuse`)
  - [x] Corrupt/partial `sessions.json` never crashes a server (fail-soft `[]`, atomic temp+rename write, dead-pid prune, idempotent self re-register). Evidence: `foxcode/channel/launch/registry.test.mjs` (`readRegistry returns [] on corrupt/partial JSON`; `register prunes dead-pid entries`; `atomic write leaves no .tmp file`)
  - [~] Tier-3/manual: 2 concurrent same-folder sessions (dev `.mcp.json` + plugin foxcode) both `evalInBrowser` against one Firefox; 2-folder isolation; owner-crash relaunch. Pending manual run on macOS (DoD).
