...
```json
{
  "mcp": {
    "foxcode": {
      "type": "local",
      "command": ["npx", "-y", "foxcode-channel@0.21.0"],
...
4. **Force a specific port.** Set `FOXCODE_PORT` env var in `.mcp.json`:
   ...
   ```
5. **Verify the channel resolves from npm:** `npx -y foxcode-channel@0.21.0 --version` should print the pinned version. A network or registry error here means npx cannot fetch the package — fix DNS/registry before re-running the launch skill.
